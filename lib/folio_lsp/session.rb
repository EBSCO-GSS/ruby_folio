...
module EBSCO
  module FOLIO
    ...
    class Session
        ...
        def view_checkouts(userId = '')
          response = RestClient.get 'https://' + @okapi_host + @config[:loans_path] + '?offset=0&limit=100&query=' + URI.escape('(userId=' + userId + ' AND status="Open")'), {:'x-okapi-token' => @okapi_token, :accept => :json}
